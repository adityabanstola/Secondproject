...
class TaskController extends Controller
{
    ...
    public function getList($key)
    {
        if (Auth::user()->role_id==1){
            ...
            }])->where('company_id',Auth::user()->company_id)->where('status',1)->orderBy('id','desc')->paginate($key);
        }
        if(Auth::user()->role_id==0){
            $tasks = Task::with(['type' => function ($query) {
                ...
                $query->select('users.id', 'users.name');
            }])->where('id',Auth::user()->id)->orderBy('id','desc')->paginate($key);
        ...
        $transformedTasks = $tasks->map(function ($task) {
            return [
                ...
                /* $task->users->value('id') */
                [
                    "id" => $task->users->value('id'),
                    "name" => $task->users->value('name'),
                ]
            ];
        });
    ...
    public function getRoosterList($key){
        if (Auth::user()->role_id==1){
            $tasks = Task::with(['type' => function ($query) {
                $query->select('id', 'type as tablename');
            }, 'location' => function ($query) {
